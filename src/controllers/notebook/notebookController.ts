...
export const verifyNotebookPassword = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        res.status(200).json({ message: SUCCESS_STRINGS.PasswordVerified });
        return;
    } catch (error) {
        next(error);
    }
...
export const previewNotebook = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    ...
    try {
        const notebook = await Notebook.findById(notebookId).select("title coverImageUrl visibility updatedAt");
        if (!notebook) {
            res.status(404).json({ error: ERROR_STRINGS.NotebookNotFound });
            return;
        }

        res.status(200).json({ preview: notebook });
        return;
    } catch (error) {
        next(error);
    }
