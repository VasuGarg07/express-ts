...
export const getNotebooksByAuthor = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const response = await notebookService.fetchNotebooksByAuthor(req.params.name, req.query);
    res.status(200).json(response);
  } catch (error) {
    next(error);
  }
...
export const getNotebookById = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const notebook = await notebookService.fetchNotebookById(req.params.id);
...
export const updateNotebook = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const notebook = await notebookService.editNotebook(req.user!.id, req.params.id, req.body);
...
export const deleteNotebookById = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const blogsDeleted = await notebookService.removeNotebookById(req.user!.id, req.params.id);
